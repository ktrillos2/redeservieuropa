import { ensureAndGetGeneralInfo, ensureAndGetHeader, getToursForMenu, getTransfersForMenu } from '@/sanity/lib/general'
...
export default async function HeaderServer() {
  ...
  const tours = await getToursForMenu()
  const transfers = await getTransfersForMenu()
  ...
  return (
    <Header 
      siteTitle={headerDoc?.siteTitle || info?.siteTitle} 
      siteSubtitle={headerDoc?.siteSubtitle || info?.siteSubtitle} 
      logoUrl={logoUrl}
      tours={tours}
      transfers={transfers}
    />
